...
object StartScriptPlugin extends Plugin {
  ...
  lazy val addStartScriptTasksCommand =
    Command.command("add-start-script-tasks") {
      (state: State) =>
        val allRefs = Project.extract(state).structure.allProjectRefs
        val allAppendSettings = allRefs.foldLeft(Seq[Setting[_]]())({
          (soFar, ref) =>
  ...
  private def relativizeFile(baseDirectory: File, f: File) = {
    if (java.io.File.separatorChar != '/') {
      ...
      } else { // leave it as-is, don't even canonicalize
        f
      }
    }
  }
  ...
  private def renderTemplate(template: String, fields: Map[String, String]) = {
    ...
    // matter
    fields.iterator.foldLeft(template)({
      (s, kv) =>
        val withAts = "@" + kv._1 + "@"
  ...
  private def relativeClasspathStringTask(baseDirectory: File, cp: Classpath) = {
    RelativeClasspathString(cp.files map {
      f => relativizeFile(baseDirectory, f)
...
exit 1
  ...
  def startScriptJettyHomeTask(streams: TaskStreams, target: File, jettyURLString: String, jettyChecksum: String) = {
    try {
      ...
      for (deleteContentsOf <- (Seq("contexts", "webapps") map {
        jettyHome / _
      })) {
        val contents = PathFinder(deleteContentsOf) ** new SimpleFileFilter({
          f =>
            f != deleteContentsOf
        })
  ...
  val startScriptForWarSettings: Seq[Project.Setting[_]] = Seq(
    ...
    startScriptJettyURL in Compile <<= (startScriptJettyVersion in Compile) {
      (version) => "http://archive.eclipse.org/jetty/" + version + "/dist/jetty-distribution-" + version + ".zip"
    },
